...
reset(arg) {
  ...
  if(!present("weapon",this_object())){
    ...
    if(a==1){
      ...
      weapon->set_short("A gnarley club");
      weapon->set_long("The gnarley club looks like a good weapon.\n");
    }
    if(a==2){
  ...
  dest_dir = 
    ({
      "/domains/areas/city/cemetary/room/hole_west1", "leave",
    });
}
init(){
  ::init();
