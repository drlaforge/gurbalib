...
add_object("domains/areas/city/cemetary/obj/bloodkey");
    long_desc = 
    ...
    dest_dir = ({
        "/domains/areas/city/cemetary/room/coffin_end", "south",
        });
    items = ({
      ({"ground","floor"}),"You are standing on it right now!",
      ({ "chamber" , "large chamber" }),
      "It looks that noone has been here in ages. Cobwebs in the corners",
      ({ "star hole", "a hole", "star shaped hole" , "hole" }), "It is shaped like a star",
...
object ob;
  ...
  writef("The hole is star shapped, so putting something shaped like a star in there would be a good start!\n");
  return 1;
}
bullet(){
...
take(str){
  ...
  if(str=="amulet"||str=="amulet from bones"){
    ...
    amu->set_name("amulet");
    amu->set_alias("scavamulet");
    amu->set_short("a silver amulet");
    amu->set_long_f("An amulet made of silver, with a flying eagle "+
	"on the front. There seem to be some electronics on the "+
	"back, and a faint hum signaling some kind of electro "+
	"magnetic shielding.\n");
    amu->set_type("amulet");
    amu->set_class(6);
    amu->set_value(100+random(500));
    amu->set_weight(2);
    if(transfer(amu,this_player())){ write("You can't carry it so you drop it on the floor.\n"); move_object(amu,this_object()); }
    amulet = 1;
   return 1;
  }
}
