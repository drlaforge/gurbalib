...
/* Snickher the Old resurrected! 100217 */
...
inherit "/std/room";
object          ob, snick, freekill, pumpkin;
string          myname;
...
query_time(){ return time_from_reset; }
void setup(void)
{
    object          ob;
    if (time_from_reset)
...
/*
 ...
 move_object(clone_object("/players/sigmund/tree/tree"), this_object());
*/
    ...
    move_object(ob, this_object());
		...
		if (!snick) {
    	snick = clone_object("/players/snickher/snickher.c");
    	move_object(snick, this_object());
		}

		if (FREE_KILL && !freekill) {
			freekill = clone_object(CHURCH_OBJECT);
			move_object(freekill, this_object());
		}

		/* Halloween pumpkin object */
/*		if (FREE_KILL && !pumpkin) {
			pumpkin = clone_object("/domains/freekill/obj/pumpkin_collector");
			move_object(pumpkin, this_object());
		}
*/

/*
    if(!present("snick", this_object()))
    move_object(clone_object("/obj/snickher/snickher"),this_object());
*/
	...
	"domains/Gangs/TG/room/main", "down", "#halloween",
		});
}
...
show_clock()
{
    write("Nuclear War uptime    : " + dstring(dtime(uptime(), 0)) + "\n");
