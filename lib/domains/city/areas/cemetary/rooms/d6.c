...
reset(arg) {
  ...
  if(!present("my_man",this_object()))
    ...
    dest_dir = ({
        "/domains/areas/city/cemetary/room/d5", "west",
        });
    if (arg) return;
  ...
  if(!fallen)
    {
      ...
      writef("The man says: Thanks for rescuing me! I thought I would "+
          "be trapped in there forever!\n",78,14);
      write("The man gives you some money.\n");
      write("The man hurries away west.\n");
      this_player()->add_money(100+random(400));
	...
	+ "The wall to the north has a hole in it.\n";
        "/domains/areas/city/cemetary/room/w4"->foo();
        return 1;
    }
